...
import type { Employee, EmployeeListFilters, EmployeeStats } from '@/types/nhms/human-resource';
import { EmployeeStatus, EmployeeType, ShiftType } from '@/types/nhms/human-resource';
import { EMPLOYEE_TABLE_HEADERS, STATUS_VARIANT_MAP, EMPLOYEE_TYPE_COLOR_MAP } from '../consts';
...
const filteredEmployees = computed(() => {
  return employees.value.filter((emp) => {
    const matchesSearch =
      emp.fullName.toLowerCase().includes(filters.value.searchQuery?.toLowerCase() || '') ||
      emp.email.toLowerCase().includes(filters.value.searchQuery?.toLowerCase() || '');
    ...
    const matchesStatus = !filters.value.status || emp.status === filters.value.status;
    const matchesType =
      !filters.value.employeeType || emp.employeeType === filters.value.employeeType;
    const matchesShift = !filters.value.shiftType || emp.shiftType === filters.value.shiftType;
...
const stats = computed<EmployeeStats>(() => {
  return {
    ...
    active: employees.value.filter((e) => e.status === EmployeeStatus.Active).length,
    fullTime: employees.value.filter((e) => e.employeeType === EmployeeType.FullTime).length,
    avgRating: (
      employees.value.reduce((sum, e) => sum + e.performance.rating, 0) / employees.value.length
    ).toFixed(1),
  };
});
</script>
      ...
      >
        ...
        <template #item.roleInfo="{ item }">
          <div class="d-flex flex-column gap-1">
            ...
            <span class="text-xs text-medium-emphasis">{{ item.department }}</span>
          </div>
        </template>

        <template #item.employmentInfo="{ item }">
          <div class="d-flex flex-column gap-1">
            <VChip :color="EMPLOYEE_TYPE_COLOR_MAP[item.employeeType]" size="x-small">
              {{ item.employeeType }}
            </VChip>
            <span class="text-xs text-medium-emphasis">{{ item.shiftType }} Shift</span>
          </div>
        </template>
        ...
        <template #item.performance="{ item }">
          <div class="d-flex align-center gap-2">
            <VRating
              ...
              color="warning"
            />
            <span class="text-body-2 font-weight-medium">{{ item.performance.rating }}</span>
          </div>
        </template>
        ...
        <template #item.actions="{ item }">
          <div class="d-flex gap-1">
            ...
            <VBtn icon variant="text" size="small" color="error" @click="deleteEmployee(item.id)">
        ...
        <template #bottom>
          <VDivider />
          <div class="d-flex justify-space-between align-center pa-4 flex-wrap gap-3">
